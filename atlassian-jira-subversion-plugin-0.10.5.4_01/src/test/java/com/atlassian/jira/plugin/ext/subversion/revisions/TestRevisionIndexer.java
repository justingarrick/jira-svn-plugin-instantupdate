...
public class TestRevisionIndexer extends MockObjectTestCase
{
    ...
    protected RevisionIndexer getRevisionIndexer()
    {
        return new RevisionIndexer(
                ...
                versionManager, issueManager, permissionManager, changeHistoryManager, serviceManager, indexPathManager);
    }
    ...
    public void testUpdateIndexWithNoIndexDirectorWhenEverythingIsOk() throws IOException, IndexException
    {
        ...
        svnLogEntry = new SVNLogEntry(Collections.EMPTY_MAP, 0, "dchui", new Date(), "TST-1");
        ...
        mockSvnMgr.expects(atLeastOnce()).method("getLogEntries").with(eq(-1L)).will(returnValue(EasyList.build(svnLogEntry)));
        ...
        revisionIndexer = new RevisionIndexer(multipleSubversionRepositoryManager,
                null, null, null, null, null, indexPathManager)
        {
        ...
        IndexReader indexReader = LuceneUtils.getIndexReader(revisionIndexer.getIndexPath());
    ...
    public void testUpdateIndexWithNoIndexDirectoryWhenEntryHasEmptyCommitMessage() throws IOException, IndexException
    {
        ...
        /* Setup the repository to return one svn entry (revision 0) */
        svnLogEntry = new SVNLogEntry(Collections.EMPTY_MAP, 0, "dchui", new Date(), StringUtils.EMPTY);
    ...
    public void testUpdateIndexWithNoIndexDirectoryWhenEntryCommitMessageHasNoJiraKeys() throws IOException, IndexException
    {
        ...
        /* Setup the repository to return one svn entry (revision 0) */
        svnLogEntry = new SVNLogEntry(Collections.EMPTY_MAP, 0, "dchui", new Date(), "There is no JIRA key here.");
...
//        assertTrue("The RevisionIndexer probably blew up attempting to index the first repo (SVN-158)", mildManneredRepo.isMethodsCalled());
    ...
    public void testGetLogEntriesByRepositoryWhenIssueKeyDoesNotMatch() throws IOException, IndexException
    {
        ...
        indexWriter = LuceneUtils.getIndexWriter(getIndexPath(), true, RevisionIndexer.ANALYZER);

        doc = new Document();
        doc.add(new Field(RevisionIndexer.FIELD_ISSUEKEY, "TST-2", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_DATE, DateField.dateToString(new Date()), Field.Store.YES, Field.Index.UN_TOKENIZED));
    ...
    public void testGetLogEntriesByRepositoryWhenRevisionCannotBeFoundInRepository() throws IOException, IndexException
    {
        ...
        doc = new Document();
        doc.add(new Field(RevisionIndexer.FIELD_ISSUEKEY, "TST-1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REPOSITORY, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REVISIONNUMBER, "0", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_DATE, DateField.dateToString(new Date()), Field.Store.YES, Field.Index.UN_TOKENIZED));
        indexWriter.addDocument(doc);
        indexWriter.flush();
        indexWriter.close();
    ...
    public void testGetLogEntriesByRepositorySorted() throws IOException, IndexException
    {
        ...
        doc.add(new Field(RevisionIndexer.FIELD_ISSUEKEY, "TST-1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REPOSITORY, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REVISIONNUMBER, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_DATE, DateField.dateToString(new Date(nowInMillis + 1000)), Field.Store.YES, Field.Index.UN_TOKENIZED));
        indexWriter.addDocument(doc);
        ...
        doc.add(new Field(RevisionIndexer.FIELD_REPOSITORY, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REVISIONNUMBER, "0", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_DATE, DateField.dateToString(new Date(nowInMillis)), Field.Store.YES, Field.Index.UN_TOKENIZED));
        indexWriter.addDocument(doc);
    ...
    public void testGetLogEntriesByProjectWhenUserHasNoPermissionToViewAnyIssuesMentionedInSvnCommit() throws IOException, IndexException
    {
        ...
        /* This is the first matching document with revision 1 */
        doc = new Document();
        doc.add(new Field(RevisionIndexer.FIELD_PROJECTKEY, "TST", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_ISSUEKEY, "TST-1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REPOSITORY, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_REVISIONNUMBER, "1", Field.Store.YES, Field.Index.UN_TOKENIZED));
        doc.add(new Field(RevisionIndexer.FIELD_DATE, DateField.dateToString(new Date()), Field.Store.YES, Field.Index.UN_TOKENIZED));
    ...
    public void testGetLogEntriesByVersionWhenUserHasNoPermissionToViewAnyIssuesOfVersionSpecified() throws IOException, IndexException
    {
        ...
        issue = new MockGenericValue("Issue", EasyMap.build("key", "TST-1"));

        mockVersionManager.expects(once()).method("getFixIssues").withAnyArguments().will(returnValue(EasyList.build(issue)));
        mockVersionManager.expects(once()).method("getAffectsIssues").withAnyArguments().will(returnValue(EasyList.build(issue)));
    ...
    public void testAddRepository() throws IOException, IndexException
    {
        ...
        entry = new SVNLogEntry(Collections.EMPTY_MAP, 0, "dchui", new Date(), "TST-1");
        mockSvnMgr.expects(atLeastOnce()).method("getId").withNoArguments().will(returnValue(1L));
        mockSvnMgr.expects(atLeastOnce()).method("isActive").withNoArguments().will(returnValue(true));
        mockSvnMgr.expects(once()).method("getLogEntries").withAnyArguments().will(returnValue(EasyList.build(entry)));
        ...
        revisionIndexer.addRepository(svnMgr);

        indexReader = LuceneUtils.getIndexReader(getIndexPath());
    ...
    public void testRemoveEntries() throws IOException, IndexException, SVNException
    {
        ...
        mockSvnMgr.expects(atLeastOnce()).method("getId").withNoArguments().will(returnValue(1L));
        mockSvnMgr.expects(atLeastOnce()).method("isActive").withNoArguments().will(returnValue(true));
        mockSvnMgr.expects(atLeastOnce()).method("getLogEntries").withAnyArguments().will(returnValue(EasyList.build(entry)));
        ...
        revisionIndexer.removeEntries(svnMgr);
        indexReader = LuceneUtils.getIndexReader(getIndexPath());
